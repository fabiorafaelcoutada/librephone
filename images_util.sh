...
create_dirtree() {
	...
	build=$(echo ${line} | cut -d ':' -f 2)
	model=$(echo ${line} | cut -d ':' -f 3)
	if test ! -e ${images}/${vendor}/${model}; then
	   ${dryrun} mkdir -p ${images}/${vendor}/${model}
	   ${dryrun} ln -s ${images}/${vendor}/${model} ${build}
	fi
    done < ${devlist}
}
...
get_metadata() {
    ...
    # Safely output metadata delimited by a pipe to avoid eval and command injection
...
move_zip() {
    for zip in *.zip; do
	IFS='|' read -r vendor model meta_build <<< "$(get_metadata "${zip}")"
	build=$(echo "${zip}" | tr -d '\n' | cut -d '-' -f 5)
 	echo "Processing ${build}"
	echo "FIXME: ${vendor}"
	dir="${images}/${vendor}/${model}"
 	if test ! -d "${dir}"; then
	    ${dryrun} mkdir -p "${images}/${vendor}/${model}"
	fi
	if test ! -d "${images}/${vendor}/${build}"; then
	    ${dryrun} ln -s "${images}/${vendor}/${model}" "${build}"
	fi
	${dryrun} mv "${zip}" "${dir}/"
...
clone() {
	...
	# if test -z $(ls -A "${images}/blobs"/${blddir}); then
	#     logmsg "EMPTY"
	# fi
	${dryrun} extractor -v -z ${images}/${blddir}
	${dryrun} extractor -v -e ${images}/${blddir}
	${dryrun} extractor -v -c ${images}/${blddir} -o ${images}/blobs
	${dryrun} extractor -v -r ${images}/${blddir}
	total=$(expr ${total} - 1)
	logmsg "${total} devices left to build..."
    done
}
...
usage() {
    ...
    echo "--zip (-z): Move lineage zip files to the right directory"
