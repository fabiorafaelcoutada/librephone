...
class UpdateDevice(object):
    ...
    def set_column(
        ...
        # Use psycopg.sql module to safely format dynamic column names preventing SQL injection
        sql_query = sql.SQL("UPDATE devices SET {} = %s WHERE build=%s").format(sql.Identifier(column))
        # print(f"SQL: {sql_query.as_string(self.dbshell)}")
        self.dbcursor.execute(sql_query, (value, build))
        return True
    ...
    def set_columns(
        ...
        for key, value in update_vals.items():
            ...
            if not clean_key.isalnum():
                ...
                return False
            # Use psycopg.sql.Identifier for dynamic keys
            set_clauses.append(sql.SQL("{} = %s").format(sql.Identifier(key)))
            sql_params.append(value)

        sql_params.append(build)

        # Use psycopg.sql module to safely concatenate parameters preventing SQL injection
        sql_query = sql.SQL("UPDATE devices SET {} WHERE build=%s").format(
            sql.SQL(", ").join(set_clauses)
        )

        # print(f"SQL: {sql_query.as_string(self.dbshell)}")
        self.dbcursor.execute(sql_query, sql_params)
        return True
